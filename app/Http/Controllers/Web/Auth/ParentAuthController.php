...
use App\Actions\Web\WebParentLoginAction;
use App\Http\Controllers\Controller;
use App\Http\Requests\Mobile\RefreshTokenRequest;
use App\Http\Requests\Web\ParentRegisterRequest;
...
class ParentAuthController extends Controller
{
    public function __construct(
        ...
        private readonly WebParentLoginAction $loginAction,
    ...
    public function register(ParentRegisterRequest $request): JsonResponse
    {
        ...
        $result = $this->webAuthService->resolveOrRegisterParent(
            $data['phone'],
            $data['country_code'],
            $centerId
        );

        // Send OTP to the parent for verification
        $otpToken = $this->webAuthService->sendOtp($data['phone'], $data['country_code'], $centerId, true);
    ...
    public function me(Request $request): JsonResponse
    {
        ...
        $user->load(['parentLinks' => fn ($q) => $q->active(), 'parentLinks.student:id,name,phone']);
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'linked_students' => $user->parentLinks->map(fn ($link): array => [
                    'id' => $link->student->id,
    ...
    private function deny(string $errorCode): JsonResponse
    {
        $message = match ($errorCode) {
            ...
            ErrorCodes::STUDENT_INACTIVE => 'Account is inactive.',
            ErrorCodes::PARENT_PORTAL_DISABLED => 'Parent portal is not enabled for this center.',
            ErrorCodes::USER_NOT_FOUND_FOR_OTP => 'No account found for this phone number. Please register first.',
            ErrorCodes::UNAUTHORIZED => 'This account is not registered as a parent.',
