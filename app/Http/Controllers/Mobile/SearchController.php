<?php

declare(strict_types=1);

namespace App\Http\Controllers\Mobile;

use App\Http\Controllers\Controller;
use App\Http\Requests\Mobile\SearchCoursesRequest;
use App\Http\Resources\Mobile\ExploreCourseResource;
use App\Models\User;
use App\Services\Courses\CourseService;
use Illuminate\Http\JsonResponse;

class SearchController extends Controller
{
    public function __construct(private readonly CourseService $courseService) {}

    public function index(SearchCoursesRequest $request): JsonResponse
    {
        /** @var User|null $student */
        $student = $request->user();

        // For authenticated users, verify they are students
        if ($student instanceof User && $student->is_student === false) {
            return response()->json([
                'success' => false,
                'error' => [
                    'code' => 'UNAUTHORIZED',
                    'message' => 'Only students can search courses.',
                ],
            ], 403);
        }

        $query = $request->queryTerm();

        if ($query !== null) {
            $paginator = $this->courseService->search($student, $query, $request->perPage(), $request->page());

            return response()->json([
                'success' => true,
                'data' => ExploreCourseResource::collection(collect($paginator->items())),
                'meta' => [
                    'page' => $paginator->currentPage(),
                    'per_page' => $paginator->perPage(),
                    'total' => $paginator->total(),
                ],
            ]);
        }

        $courses = $this->courseService->fallback($student);

        return response()->json([
            'success' => true,
            'data' => ExploreCourseResource::collection($courses),
            'meta' => [
                'page' => 1,
                'per_page' => 5,
                'total' => $courses->count(),
            ],
        ]);
    }
}
