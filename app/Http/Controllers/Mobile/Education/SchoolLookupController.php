...
use App\Http\Requests\Mobile\Education\ListMobileSchoolsRequest;
...
{
    public function index(ListMobileSchoolsRequest $request, Center $center): JsonResponse
    {
        ...
        if (! $student instanceof User || $student->is_student === false) {
            return response()->json([
                ...
                'error' => [
                    'code' => 'UNAUTHORIZED',
                    'message' => 'Only students can access schools.',
                ],
            ], 403);
        }
        ...
        if ((bool) ($profile['enable_school'] ?? true) === false) {
            return response()->json([
                'success' => true,
                'data' => [],
            ]);
        ...
        if (isset($validated['type']) && is_numeric($validated['type'])) {
            $query->where('type', (int) $validated['type']);
        }
        ...
        return response()->json([
            'success' => true,
            'data' => SchoolResource::collection($query->get()),
        ]);
    }
}
